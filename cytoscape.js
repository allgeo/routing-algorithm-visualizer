    ...
    timeoutArr.push(
        setTimeout(() => {
            document.getElementById(
                "dvAnnotation"
            ).innerHTML = `No updates from neighbor nodes have been sent, so now each node has the shortest distance to all other nodes in the network. See graph below to see path from ${start} to ${end}`;
        }, startTimeout + 2000)
    );
...
//     let distanceVectorContainer = document.getElementById(
